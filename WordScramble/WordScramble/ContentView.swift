...
//  WordScramble
//
//  Created by Karol Harasim on 26/02/2020.
...
struct ContentView: View {
    ...
    func addNewWord() {
        ...
        guard isLongerThanThreeLetters(word: answer) else {
            wordError(title: "Word shorter than 3 letters", message: "Try harder next time!")
            score -= 1
            return
        }
        guard isNotStartWord(word: answer) else {
            wordError(title: "Word is the same as root word", message: "Be more creative!")
            score -= 1
            return
        }
